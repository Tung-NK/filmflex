            ...
            <div class="col-12">
                <div class="main__title">
                    ...
                    <div class="main__title-wrap">
                        <a href="{{ route('genres.create') }}" class="main__title-link main__title-link--wrap">Add Genres</a>
                        ...
                        <!-- search -->
                        <form action="{{ route('genres.index') }}" method="GET" class="main__title-form">
                    ...
                    <table class="catalog__table">
                        ...
                        @foreach ($data_genre as $item)
                            <tr>
                                ...
                                <td><div class="catalog__text">{{ $item->genre_name }}</div></td>
                                {{-- <td><div class="catalog__text">{{ $item->genre_poster }}</div></td> --}}
                                <td>
                                    <div class="catalog__text">
                                        <img src="{{ asset('storage/' . $item->genre_poster) }}" width="100px" height="80px" alt="{{ $item->genre_name }}">
                                    </div>
                                </td>
                                <td>
                                    <div class="catalog__text">
                                    ...
                                    <div class="catalog__btns">
                                        @if(!$item->trashed())
                                            <div style="display: flex; align-items: center;">
                                                <a href="{{ route('genres.edit', $item->id) }}" class="catalog__btn catalog__btn--edit  mb-3" style="height: 40px; padding: 0 10px; line-height: 40px;">
                                                    <i class="ti ti-edit"></i>
                                                </a>
                                                <form action="{{ route('genres.destroy', $item->id) }}" method="POST" style="display:inline; margin-left: 5px;">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        @else
                                            <div style="display: flex; align-items: center;">
                                                <form action="{{ route('genres.restore', $item->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                ...
                                                <form action="{{ route('genres.forceDelete', $item->id) }}" method="POST" style="display:inline; margin-left: 5px;" onclick="return confirm('Are you sure you want to delete this actor?');">
                                                    @csrf
                                                    @method('DELETE')
               ...
               <div class="main__paginator d-flex justify-content-between align-items-center text-white">
                   ...
                   <ul class="paginator">
                       <li class="paginator__item paginator__item--prev {{ $currentPage == 1 ? 'disabled' : '' }}">
                           <a href="{{ $currentPage > 1 ? route('genres.index', ['page' => $currentPage - 1, 'search' => request('search')]) : '#' }}">
                       ...
                       @for ($i = 1; $i <= $totalPages; $i++)
                           <li class="paginator__item {{ $i == $currentPage ? 'paginator__item--active' : '' }}">
                               <a href="{{ route('genres.index', ['page' => $i, 'search' => request('search')]) }}">{{ $i }}</a>
                           </li>
                       @endfor

                       <li class="paginator__item paginator__item--next {{ $currentPage == $totalPages ? 'disabled' : '' }}">
                           <a href="{{ $currentPage < $totalPages ? route('genres.index', ['page' => $currentPage + 1, 'search' => request('search')]) : '#' }}">
